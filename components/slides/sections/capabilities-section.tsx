...
import type { Locale, CopyData, Capability } from '@/components/marketing/data/types';
import { capabilities } from '@/components/marketing/data/capabilities';
...
interface CapabilitiesSectionProps { locale: Locale; t: CopyData; cardIndex?: number; }

export function CapabilitiesSection({ locale, t, cardIndex }: CapabilitiesSectionProps) {
  const capabilityItems = capabilities[locale];
  const items = cardIndex !== undefined ? [capabilityItems[cardIndex]] : capabilityItems;
  const indices = cardIndex !== undefined ? [cardIndex] : capabilityItems.map((_, i) => i);
  ...
  return (
    <section id="platform" className="mx-auto w-full max-w-7xl px-6 pb-24 pt-28 lg:px-12">
      <div data-reveal className="reveal-item" style={getRevealStyle(60)}>
        <p className="text-xs uppercase tracking-[0.16em] text-zinc-400">{t.capabilitiesTitle}</p>
        {cardIndex !== undefined && (
          <h2 className="mt-3 text-3xl font-semibold text-zinc-100">{items[0].title}</h2>
        )}
        {cardIndex === undefined && (
      ...
      <div className={cn('mt-8 grid grid-cols-1 gap-4', cardIndex === undefined && 'md:grid-cols-6')}>
        {items.map((item, i) => (
          <BentoCard key={item.title} item={item} index={indices[i]} />
        ))}
      </div>
    </section>
  );
